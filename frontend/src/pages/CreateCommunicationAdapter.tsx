...
export const CreateCommunicationAdapter = () => {
  ...
  useEffect(() => {
    if (location.state?.adapter && location.state?.isEdit) {
      ...
      const frontendMode = adapter.mode === 'SENDER' ? 'sender' : 'receiver';
  ...
  const handleSaveAdapter = async () => {
    ...
    if (adapterMode === 'receiver') {
      // Receiver adapters send data TO external systems
      ...
      if (selectedAdapter === 'rest') {
        if (!configuration.targetEndpointUrl) {
          toast({
            title: "Validation Error",
            description: "Target Endpoint URL is required for REST Receiver adapter",
            variant: "destructive",
          });
          return;
        }
        if (!configuration.httpMethod) {
          toast({
            title: "Validation Error",
            description: "HTTP Method is required for REST Receiver adapter",
            variant: "destructive",
          });
          return;
        }
        if (!configuration.contentType) {
          toast({
            title: "Validation Error",
            description: "Content Type is required for REST Receiver adapter",
            variant: "destructive",
          });
          return;
        }
      ...
      // Sender adapters receive data FROM external systems
      // Check for sender-specific required fields based on adapter type
      if (selectedAdapter === 'rest') {
        if (!configuration.port) {
          toast({
            title: "Validation Error",
            description: "Port is required for REST Sender adapter",
            variant: "destructive",
          });
          return;
        }
        ...
        if (!configuration.serviceEndpointUrl) {
          toast({
            title: "Validation Error",
            description: "Endpoint URL is required for SOAP Sender adapter",
            variant: "destructive",
          });
          return;
        }
    ...
    // Sender Adapter = Receives data FROM external systems (OUTBOUND)
    // Receiver Adapter = Sends data TO external systems (INBOUND)
